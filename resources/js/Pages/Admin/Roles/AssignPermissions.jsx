...
export default function AssignPermissions({ role, permissions }) {
    ...
    const handleParentChange = (parentPrefix, isChecked) => {
        ...
        const childrenIds = groupedPermissions[parentPrefix].children.map(
            (c) => c.id
        );

        if (isChecked) {
            updatedPermissions = [
                ...new Set([...updatedPermissions, ...childrenIds]),
            ];
        } else {
    ...
    const handleChildChange = (childId, parentPrefix, isChecked) => {
        ...
        const allChildrenSelected = groupedPermissions[
            parentPrefix
        ].children.every((c) => updatedPermissions.includes(c.id));
        if (allChildrenSelected) {
            updatedPermissions.push(
                ...groupedPermissions[parentPrefix].children.map((c) => c.id)
            );
        }
    ...
    const getParentState = (parentPrefix) => {
        const childIds = groupedPermissions[parentPrefix].children.map(
            (c) => c.id
        );
    ...
    const updatePermissions = (permissions) => {
        post(
            route("admin.roles.assign-permissions.store", role.id),
            { permissions },
            { preserveScroll: true }
        );
    };
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        <div className="flex items-center justify-end mt-6">
                            <button
                                ...
                                className="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 transition duration-200 ease-in-out"
                                disabled={processing} // Ensure processing state is updated correctly
                            >
                                {processing
                                    ? "Saving..."
                                    : "Assign Permissions"}
